    ...
    const handleContact = async () => {
        ...
        try {
            ...
            if (
                ...
                router.push(
                    `/dashboard/messages/${(conversation as { id: string }).id}`
                );
                return;
            }
            ...
            if (
                (typeof errorObj?.response?.data === "object" &&
                    ...
                    typeof (errorObj.response.data as { message?: string })
                        .message === "string" &&
                    (
                        errorObj.response.data as { message: string }
                    ).message.includes("already exists")) ||
                errorObj?.response?.status === 409
            ) {
                try {
                    const conversations = await apiService.get(
